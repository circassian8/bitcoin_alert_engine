...
def test_symmetric_registry_includes_reversal_experiment() -> None:
    root = Path(__file__).resolve().parents[1]
    registry = load_research_registry(root / "research_registry_smoke_symmetric.yaml")
    reversal = [exp for exp in registry.experiments if exp["generator"] == "stress_reversal_v0"]
    assert reversal
    assert reversal[0]["sides"] == ["long", "short"]
