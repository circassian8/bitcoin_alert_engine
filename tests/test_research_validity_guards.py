...
from btc_alert_engine.research.experiments import ExperimentDataset, build_experiment_event_frame, feature_columns_for_experiment
...
def test_feature_columns_for_micro_score_excludes_gate_columns() -> None:
    frame = pd.DataFrame(
        {
            "micro_bybit_score__ofi_60s": [1.0, 2.0],
            "micro_bybit_score__gate_pass": [True, False],
            "micro_bybit_score__gate_pass_long": [True, False],
            "micro_bybit_score__gate_pass_short": [False, True],
        }
    )
    columns = feature_columns_for_experiment(frame, ["micro_bybit_score"])
    assert columns == ["micro_bybit_score__ofi_60s"]


def test_generate_verification_report_warns_when_reports_exist_but_inputs_are_missing(tmp_path: Path) -> None:
    registry = {
        "program": {"id": "wf_checks", "version": 1},
        "execution": {"venue": "bybit", "symbol": "BTCUSDT", "category": "linear", "mode": "alert", "market_truth": "bybit_only"},
        "goal": {"primary_metric": "expectancy_r_per_alert", "production_budget_per_week": 3},
        ...
        "validation": {
            "warmup_only": {"start": "2025-01-01", "end": "2025-01-31"},
            "outer_walk_forward": {"train_days": 30, "calibrate_days": 10, "test_days": 10, "purge_hours": 24, "embargo_hours": 24},
            "final_frozen_test": {
                "freeze_date": "2025-06-01",
                "calibrate_window": {"start": "2025-06-02", "end": "2025-06-12"},
                "untouched_test": {"start": "2025-06-13", "end": "2025-06-23"},
    ...
    registry_path = tmp_path / "registry.yaml"
    registry_path.write_text(yaml.safe_dump(registry), encoding="utf-8")
    reports_dir = tmp_path / "data" / "reports" / "walkforward"
    reports_dir.mkdir(parents=True)
    (reports_dir / "summary_metrics.csv").write_text("experiment_id\nA0\n", encoding="utf-8")
    from btc_alert_engine.verification.project import generate_verification_report

    report = generate_verification_report(
        data_dir=tmp_path / "data",
        registry_path=registry_path,
        contracts_path=Path(__file__).resolve().parents[1] / "feature_contracts.yaml",
        symbol="BTCUSDT",
    )
    codes = {item.code for item in report.static_checks}
    assert "reports_exist_but_inputs_missing" in codes
